...
Requirements: Drupal 6.x, doji_gold Framework downloaded from doji_goldcss.org


--- INSTALLATION --------------------------------------------------------

1. Place the doji_gold folder in your themes directory.

2. Download doji_gold http://www.doji_goldcss.org/
   a. Extract folder, creating something like "joshuaclayton-doji_gold-css-28c8aa9ae2686442e00a5c7f46dfe2de76b3bd83"
   b. Move folder to /sites/all/libraries and rename to "doji_gold".
   c. Ensure your path looks like /sites/all/libraries/doji_gold/doji_gold/screen.css
   
   * Optionally, doji_gold now also supports the Libraries module (http://drupal.org/project/libraries) so you can put your doji_gold framework files in any of the locations that libraries supports. 
...
- uses doji_gold 0.8 (or greater) CSS framework: http://www.doji_goldcss.org/
- normalizes Drupal's CSS to be consistent
- properly aggregates all doji_gold CSS files into a single file when this setting is enabled
...
- (performance) apply system.css.patch to remove uncessary HTTP requests to images that doji_gold overrides already



--- USING doji_gold --------------------------------------------------------

doji_gold aligns designs to a grid. If you add a class "showgrid" to any <div class="container showgrid"> it will
show you the grid it is working with, both in terms of columns and rows. You can read more by following the links
on the homepage: http://www.doji_goldcss.org/

To get doji_gold to work with Drupal (because Drupal adds in paddings, borders, and more), we need to override
...
--- NOTE: -----------------------------------------------------------
doji_gold theme now includes two ways of adding a logo: via the $logo variable or via css. There is a setting in the theme settings to choose your method.
