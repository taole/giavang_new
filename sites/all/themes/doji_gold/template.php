...
if (theme_get_setting('doji_gold_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}
if (theme_get_setting('doji_gold_animated_submit')) {
  drupal_add_js(drupal_get_path('theme', 'doji_gold') .'/scripts/submit_animated.js');
}

/**
 * Implements HOOK_theme().
 */
function doji_gold_theme(&$existing, $type, $theme, $path) {
  if (!db_is_active()) {
    return array();
  }
  include_once './' . drupal_get_path('theme', 'doji_gold') . '/template.theme-registry.inc';
  return _doji_gold_theme($existing, $type, $theme, $path);
}
 ...
 */
function doji_gold_preprocess_page(&$vars) {
    if (!empty($vars['node']) && !empty($vars['node']->type)) {
        $vars['template_files'][] = "page-node-" . $vars['node']->type;
    }
  global $user;
  $vars['path'] = base_path() . path_to_theme() .'/';
  $vars['path_parent'] = base_path() . drupal_get_path('theme', 'doji_gold') . '/';
  $vars['user'] = $user;

  // Prep the logo for being displayed
  $logo2_path = $vars['path'] . 'logo2.png';

  if (file_exists(trim($logo2_path, '/'))) {    
    $vars['logo2_path'] = l("<img src='" . $logo2_path . "' alt='". $title ."' border='0' />", '<front>', array('attributes' => array('title' => $title . $site_slogan, 'class' => 'site-logo2'), 'html' => TRUE));
  }
  ...
  if ($vars['is_front'] && $vars['mission'] != '') {
    $vars['meta'] .= '<meta name="description" content="'. doji_gold_trim_text($vars['mission']) .'" />'."\n";
  }
  elseif (isset($vars['node']->teaser) && $vars['node']->teaser != '') {
    $vars['meta'] .= '<meta name="description" content="'. doji_gold_trim_text($vars['node']->teaser) .'" />'."\n";
  }
  elseif (isset($vars['node']->body) && $vars['node']->body != '') {
    $vars['meta'] .= '<meta name="description" content="'. doji_gold_trim_text($vars['node']->body) .'" />'."\n";
  }
  ...
  if (theme_get_setting('doji_gold_showgrid')) {
    $vars['body_classes'] .= ' showgrid ';
  }

  //Setup the vars for the doji_gold Libraries location.
  if (module_exists('libraries')) {
    $vars['bp_library_path'] = module_invoke('libraries', 'get_path', 'doji_gold') .'/';
  }
  else {
    $vars['bp_library_path'] = 'sites/all/libraries/doji_gold/';
  }

  //Add the screen and print css files
  drupal_add_css($vars['bp_library_path'] .'doji_gold/screen.css', 'theme', 'screen,projection');
  $vars['css'] = drupal_add_css($vars['bp_library_path'] .'doji_gold/print.css', 'theme', 'print');

  //Perform RTL - LTR swap and load RTL Styles.
  if ($vars['language']->dir == 'rtl') {
    // Remove doji_gold Grid and use RTL grid
    $css = $vars['css'];
    $css['screen,projection']['theme'][$vars['bp_library_path'] .'/doji_gold/plugins/rtl/screen.css'] = TRUE;
  ...
  $vars['css_alt'] = doji_gold_css_reorder($vars['css']);
  ...
  */
  ...
  if (arg(0) == 'node' && isset($vars['node']->type) && $vars['node']->type== 'goldprice') {
    $vars['title'] = NULL;
  }
  
  if (arg(0) == 'node' && arg(1) == '11376' && arg(2) == 'edit') {
    $vars['tabs'] = NULL;    
  }
 ...
 *   A sequential array of variables passed to the theme function.
 */
function doji_gold_preprocess_node(&$vars) {
 ...
 *   A sequential array of variables passed to the theme function.
 */
function doji_gold_preprocess_comment(&$vars) {
 ...
 */
function doji_gold_preprocess_block(&$vars, $hook) {
  $block = $vars['block'];
  ...
  if (theme_get_setting('doji_gold_block_edit_links') && user_access('administer blocks')) {
    include_once './' . drupal_get_path('theme', 'doji_gold') . '/template.block-editing.inc';
    doji_gold_preprocess_block_editing($vars, $hook);
 ...
 *   A sequential array of variables passed to the theme function.
 */
function doji_gold_preprocess_box(&$vars) {
 ...
 */
function doji_gold_username($object) {
 ...
 */
function doji_gold_breadcrumb($breadcrumb) {
 ...
 */
function doji_gold_form_element($element, $value) {
  $args = func_get_args();
  return preg_replace('@([.!?]):\s*(</label>)@i', '$1$2', call_user_func_array('theme_form_element', $args));
}

/**
 * Set status messages to use doji_gold CSS classes.
 */
function doji_gold_status_messages($display = NULL) {
  $output = '';
  foreach (drupal_get_messages($display) as $type => $messages) {
    // doji_gold can either call this success or notice
    if ($type == 'status') {
      $type = 'success';
    }
 ...
 */
function doji_gold_comment_wrapper($content, $node) {
  global $user;
  $output = '';
 ...
 */
  ...
  function doji_gold_forum_icon($new_posts, $num_posts = 0, $comment_mode = 0, $sticky = 0) {
 ...
 */
function doji_gold_forum_topic_navigation($node) {
  return '';
}
}
 ...
 */
function doji_gold_trim_text($text, $length = 150) {
 ...
 * weighted styles.
 */
function doji_gold_css_reorder($css) {
