 ...
 * and decimal point.
 *
 * Formatting options are expected to be at Drupal.settings.doji.
 *
 * @param float number
...
Drupal.formatNumber = function(number, decimals, truncate) {
  ...
  if (decimals > 0) {
    ...
    number += Drupal.settings.doji.decimal_point + decimal_part;
  }
  ...
  // min/max fields in CCK field settings.
    ...
    number += Drupal.settings.doji.decimal_point + number_parts[1];
  }
...
Drupal.parseNumber = function(number, required) {
  ...
  if (Drupal.settings.doji.thousands_sep.length > 0) {
    var thsep = Drupal.settings.doji.thousands_sep;
    if (thsep == '\u00A0') {
      thsep += ' ';
    }
  ...
  if (Drupal.settings.doji.decimal_point != '.') {
    number = number.replace(new RegExp('[' + Drupal.settings.doji.decimal_point + ']', 'g'), '.');
  }
...
Drupal.numericElement.clearThousandsSep = function($element) {
  ...
  if (number.length > 0 && Drupal.settings.doji.thousands_sep.length > 0) {
    var thsep = Drupal.settings.doji.thousands_sep;
    if (thsep == '\u00A0') {
      thsep += ' ';
    }
