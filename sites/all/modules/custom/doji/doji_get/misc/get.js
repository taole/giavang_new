...
Drupal.behaviors.doji_get = function(context) {
  $('#giavangquocte', context).everyTime(1000, 'gia_vang_quoc_te', function(i) {
    var $this = $(this);
    $.get('/?q=doji/get/json/gia_vang_quoc_te', null,
      function(res){  
        if (!res.error && res.text) {
          $this.find('table.goldprice-view').html(res.text);  
          ...
          data = res.data;                             
          for (i in data) {
            $tr = $this.find('table.goldprice-view tbody tr:eq(' + i + ')');
            $tr.toggleClass('blind', 1000);
            $tr.html('');
            $tr.append('<td class="first">'+ data[i]['label']['data'] +'</td>');
            $tr.append('<td>'+ data[i]['bid'] +'</td>');
            $tr.append('<td>'+ data[i]['ask'] +'</td>');
            $tr.append('<td>'+ data[i]['percent'] +'</td>');
            $tr.append('<td>'+ data[i]['spread_value'] +'</td>');    
            $tr.toggleClass('blind', 1000);                                            
          }
        }
      }
