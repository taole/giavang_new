...
function Chart(options, callback) {
	...
	function Axis(userOptions) {
      ...
      if (options.customControl == 'tvazone') {
        min = mathFloor(min/100)*100;
        max = mathFloor(max/100)*100 + 100;
        tickInterval =  mathFloor((max-min)/10);             
        j = min;
        tickPositions = [];
...
Highcharts.RangeSelector = function (chart) {
	...
	function render(min, max) {
      ...
      */      
        $('.doji-charts-'+i).click(function(){
          $('#chart-buttons ul.chartbuttons li a').removeClass('chart-actived');
          $(this).addClass('chart-actived');
          clickButton(i, rangeOptions); 
        });        
			});
